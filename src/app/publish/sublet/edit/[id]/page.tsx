...
import { splitPhoneForInput } from "@/lib/phone/validatePhone";
import {
...
export default function SubletEditPage() {
  ...
  const [contactPhoneCountry, setContactPhoneCountry] =
    useState<SupportedPhoneCountry>(DEFAULT_PHONE_COUNTRY);
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          if (data.contactPhone) {
            const parts = splitPhoneForInput(data.contactPhone);
            setContactPhone(parts.nationalNumber);
            setContactPhoneCountry(parts.country);
          } else {
            setContactPhone("");
